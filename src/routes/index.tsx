...
import { estimateProjectPrice } from "@/lib/ai-price.functions";
...
type TabKey = "contabilidad" | "proyectos" | "resultados" | "radar";

function Index() {
  return <Dashboard tab="resultados" />;
}
...
/* ------------------------------ Simulador (AI-powered) ------------------------------ */

type Experience = "junior" | "mid" | "senior";
type CompanySize = "micro" | "pyme" | "grande";
...
const COMPANY_LABEL: Record<CompanySize, string> = {
  micro: "Microempresa / Autónomo",
  pyme: "PYME (Pequeña y mediana empresa)",
  grande: "Gran Empresa / Corporación",
};
...
type AIEstimate = {
  ...
  baseAmount: number;
  toolsCost: number;
  toolsBreakdown: { name: string; cost: number }[];
  priceWithVat: number;
  reasoning: string;
};
...
function ResultadosView({
  ...
  const { rows: fixedCosts } = useFixedCosts();
  const [scope, setScope] = usePersistentState("fc.res.scope", "");
  const [experience, setExperience] = usePersistentState<Experience>("fc.res.exp", "mid");
  const [companySize, setCompanySize] = usePersistentState<CompanySize>("fc.res.size", "pyme");
  const [price, setPrice] = usePersistentState<number>("fc.res.price", 0);
  ...
  const MAX_TURNS = 3;
  // turns = number of AI completion exchanges so far (each AI response = 1 turn)
  const turnsUsed = chat.filter((m) => m.role === "assistant").length;

  const estimate_fn = useServerFn(estimateProjectPrice);
  ...
  const floorHourlyRate = useMemo(() => {
    const hours = Math.max(monthlyBillableHours, 1);
    const desired = Math.max(monthlyGoal, 0);
    return (desired + fixedMonthly) / hours;
  }, [monthlyGoal, fixedMonthly, monthlyBillableHours]);
  ...
  const runEstimate = useCallback(
    async (messages: ChatMsg[]) => {
      ...
      try {
        ...
        const result = await estimate_fn({
          data: { messages, experience, companySize, turn, floorHourlyRate },
        });
        if (result.status === "needs_info") {
          const assistantMsg: ChatMsg = { role: "assistant", content: result.question };
          setChat([...messages, assistantMsg]);
          ...
          setEstimate({
            ...
            toolsCost: result.toolsCost,
            toolsBreakdown: result.toolsBreakdown ?? [],
            priceWithVat: result.priceWithVat,
            reasoning: result.reasoning,
          });
          setPendingQuestion(null);
          if (result.priceWithVat > 0) setPrice(Math.round(result.priceWithVat));
          setChat([
            ...messages,
            { role: "assistant", content: `✅ Estimación lista: ${Math.round(result.priceWithVat)}€ (IVA inc.) · ${result.estimatedHours}h.` },
          ]);
        }
      } catch (e) {
        setError((e as Error).message || "Error generando estimación");
      } finally {
        setLoading(false);
      }
    },
    [estimate_fn, experience, companySize, setPrice, floorHourlyRate],
  );
  ...
  const startConversation = useCallback(() => {
    ...
    const initial: ChatMsg[] = [
      {
        role: "user",
        content: `Descripción del proyecto: """${trimmed}"""\n\nExperiencia: ${experience}. Cliente: ${companySize}.`,
      },
    ];
    setChat(initial);
    setEstimate(null);
    setPendingQuestion(null);
    setAnswer("");
    void runEstimate(initial);
  }, [scope, experience, companySize, runEstimate]);
    ...
    <div className="space-y-6">
      <header>
        <h1 className="flex items-center gap-2 text-xl font-bold sm:text-2xl">
          <Sparkles className="h-6 w-6 text-emerald-400" /> Simulador de Tarifas
        </h1>
        <p className="text-sm text-slate-400">
          Estimador de presupuesto con IA. Analiza tu descripción y propone un precio realista para el mercado español 2026.
        </p>
      </header>
      ...
      <section className="rounded-xl border border-slate-800 bg-slate-900 p-6">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold">Define tu Proyecto</h2>
        </div>
        ...
        <div className="space-y-5">
          <div className="rounded-xl border border-indigo-500/30 bg-indigo-500/5 p-4 text-sm text-indigo-100">
            <div className="mb-2 font-semibold text-indigo-200">
              💡 Guía para optimizar el resultado de la IA
            </div>
            <ul className="space-y-1.5 text-[13px] leading-snug text-indigo-100/90">
              <li>• <span className="font-medium">Horas estimadas de ejecución:</span> Cuántas horas prevés dedicar al desarrollo y reuniones.</li>
              <li>• <span className="font-medium">Herramientas y licencias:</span> Especifica qué software de pago requiere este cliente y su coste exacto.</li>
              <li>• <span className="font-medium">Alcance y entregables:</span> Define el alcance neto (ej: número de diseños, páginas, campañas).</li>
              <li>• <span className="font-medium">Complejidad técnica:</span> Menciona si hay integraciones avanzadas o dependencias externas.</li>
              <li>• <span className="font-medium">Plazos de entrega:</span> Indica si hay urgencia o entregas en días festivos.</li>
              <li>• <span className="font-medium">Rondas de revisión:</span> Especifica cuántas correcciones están contempladas.</li>
            </ul>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-slate-400">
              Describe con detalle el alcance. La IA distingue entre un logo simple, una web completa o una estrategia SEO completa.
            </p>
            <textarea
              value={scope}
              onChange={(e) => setScope(e.target.value)}
              rows={5}
              placeholder="Ej: Aplicación web a medida con login, dashboard de métricas, pasarela de pagos Stripe e integración con un CRM externo…"
              className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-100 placeholder:text-slate-500 focus:border-emerald-500 focus:outline-none"
              disabled={chat.length > 0}
            />
            <div className="flex items-center justify-between gap-2">
              <p className="text-[11px] text-slate-500">
                {scope.trim().length < 20
                  ? `Escribe al menos ${20 - scope.trim().length} caracteres más.`
                  : chat.length > 0
                    ? `Conversación en curso (${turnsUsed}/${MAX_TURNS} turnos IA).`
              ...
              {chat.length === 0 ? (
                <button
                  ...
                  disabled={scope.trim().length < 20 || loading}
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-slate-200">Tus Años de Experiencia</label>
              <select
                value={experience}
            ...
            <div className="space-y-1.5">
              ...
              <select
                ...
                className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-100 focus:border-emerald-500 focus:outline-none"
              >
                {(Object.keys(COMPANY_LABEL) as CompanySize[]).map((k) => (
                  <option key={k} value={k}>
                    {COMPANY_LABEL[k]}
                  </option>
                ))}
          ...
          <div className="rounded-xl border border-emerald-500/40 bg-emerald-500/5 p-4">
            ...
            <div className="relative">
              <input
                ...
                placeholder="0"
                className="w-full rounded-lg border border-emerald-500/40 bg-slate-950 px-3 py-3 pr-10 text-lg font-bold text-white outline-none focus:border-emerald-500"
              />
              <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-sm text-emerald-300">
                €
              </span>
            </div>
            <p className="mt-2 text-[11px] text-emerald-200/70">
              Este precio representa el importe final que cobrarás al cliente (IVA incluido). Se inyecta automáticamente con la recomendación de la IA.
            </p>
          </div>
        </div>
      </section>
      ...
      {chat.length > 0 && (
        <section className="rounded-xl border border-slate-800 bg-slate-900 p-5">
          <div className="mb-3 flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold">Conversación con la IA</h2>
            <span className="ml-auto text-[11px] text-slate-400">
              Turno {Math.min(turnsUsed, MAX_TURNS)}/{MAX_TURNS}
            </span>
          </div>
          <div className="space-y-3">
            {chat.map((m, i) => (
              <div
                ...
                className={`rounded-lg px-3 py-2 text-sm ${
                  m.role === "user"
                    ? "ml-6 border border-slate-700 bg-slate-950 text-slate-100"
                    : "mr-6 border border-emerald-500/30 bg-emerald-500/5 text-emerald-100"
                }`}
              >
            ...
            {loading && (
              <div className="mr-6 flex items-center gap-2 rounded-lg border border-emerald-500/30 bg-emerald-500/5 px-3 py-2 text-sm text-emerald-200">
                <div className="h-3 w-3 animate-spin rounded-full border-2 border-emerald-400 border-t-transparent" />
                Pensando…
              </div>
            )}
          ...
          {pendingQuestion && !loading && (
            <div className="mt-4 space-y-2">
              <label className="text-xs font-medium text-slate-300">Tu respuesta</label>
      ...
      {estimate && !loading && (
        <section className="rounded-xl border border-indigo-500/30 bg-gradient-to-br from-indigo-500/10 to-emerald-500/10 p-6">
          <div className="mb-4 flex items-center gap-2">
            <Sparkles className="h-5 w-5 text-indigo-300" />
            <h2 className="text-lg font-semibold">Estimación IA</h2>
            <span className="ml-auto text-xs text-slate-400">
              {EXPERIENCE_LABEL[experience]} · {COMPANY_LABEL[companySize]}
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            />
            <ResultCard
              title="Tarifa dinámica IA"
              value={`${fmt(estimate.hourlyRate)}/h`}
              icon={<Gauge className="h-4 w-4" />}
              accent="indigo"
            />
            <ResultCard
              title="Total Presupuesto Final (PVP)"
              value={fmt(estimate.priceWithVat)}
          ...
          <div className="mt-4 rounded-lg border border-slate-800 bg-slate-950 p-4">
            ...
            <p className="text-sm text-slate-200">{estimate.reasoning}</p>
          </div>
          ...
          <div className="mt-4 overflow-hidden rounded-lg border border-slate-800 bg-slate-950">
            <div className="flex items-center justify-between border-b border-slate-800 px-4 py-2.5 text-sm">
              <span className="text-slate-300">
                ...
                <span className="ml-2 text-[11px] text-slate-500">
                  ({fmt(estimate.hourlyRate)}/h × {estimate.estimatedHours}h)
                </span>
              </span>
              <span className="font-semibold text-white">{fmt(estimate.baseAmount)}</span>
            </div>
            <div className="flex flex-col gap-1 border-b border-slate-800 px-4 py-2.5 text-sm">
              <div className="flex items-center justify-between">
                <span className="text-slate-300">Gastos Directos de Herramientas</span>
                <span className="font-semibold text-white">{fmt(estimate.toolsCost)}</span>
              </div>
              {estimate.toolsBreakdown.length > 0 && (
                <ul className="ml-2 mt-1 space-y-0.5 text-[11px] text-slate-500">
                  {estimate.toolsBreakdown.map((t, i) => (
                    <li key={i}>• {t.name}: {fmt(t.cost)}</li>
                  ))}
                </ul>
              )}
            </div>
            <div className="flex items-center justify-between border-b border-slate-800 px-4 py-2.5 text-sm">
              <span className="text-slate-300">
                IVA (21%)
                <span className="ml-2 text-[11px] text-slate-500">
                  sobre {fmt(estimate.baseAmount + estimate.toolsCost)}
                </span>
              </span>
              <span className="font-semibold text-amber-300">
                {fmt((estimate.baseAmount + estimate.toolsCost) * 0.21)}
              </span>
            </div>
            <div className="flex items-center justify-between bg-emerald-500/10 px-4 py-3">
              <span className="text-sm font-semibold text-emerald-200">Total Presupuesto Final (PVP)</span>
              <span className="text-xl font-bold text-white">{fmt(estimate.priceWithVat)}</span>
            </div>
          </div>

          <div className="mt-5 rounded-lg border border-amber-500/30 bg-amber-500/5 p-4 text-sm text-amber-200">
            💡 Usa este precio como referencia. Copia el valor a "Gestión de Proyectos" cuando crees el proyecto.
          </div>

          <p className="mt-3 text-[11px] text-slate-500">
            Suelo de tarifa horaria aplicado: {fmt(floorHourlyRate)}/h · Gastos fijos: {fmt(fixedMonthly)}/mes · Objetivo neto: {fmt(monthlyGoal)}/mes · Horas facturables: {monthlyBillableHours}h/mes. La IA no baja de este suelo.
          </p>
        </section>
      )}
    </div>
